                    ...
                    <div class="col-xl-12">
                        <div class="card">
                            <div class="card-header">
                                <strong class="card-title">留言管理</strong>
                            </div>
                                ...
                                <table class="table" style="margin-bottom: 0;">
                                    ...
                                    <c:forEach items="${messages}" var="message" varStatus="index">
                                        <tr>
                                            ...
                                            <td>${message.studentRealName}</td>
                                            <td>${message.studentNum}</td>
                                            <td><f:formatDate value="${message.createTime}" pattern="yyyy-MM-dd HH:mm:ss"/></td>
                                            <td>${!message.state ? "带审核" : "已通过"}</td>
                                            <td>
                                                <c:if test="${!message.state}">
                                                    <a href="${ctx}/message/update?id=${message.id}"
                                                       class="btn btn-info">通过</a>
                                                </c:if>
                                                <a href="${ctx}/message/delete?id=${message.id}" class="btn btn-danger">删除</a>
                                            </td>
                                        </tr>
                                    </c:forEach>
                                    </tbody>
