                    ...
                    <div class="col-xl-12">
                        <div class="card">
                            <div class="card-header">
                                <strong class="card-title">我的交易</strong>
                            </div>
                            <div class="table-stats order-table ov-h">
                                <table class="table" style="margin-bottom: 0;">
                                    ...
                                    <tbody>
                                    <c:forEach items="${fromTrades}" var="fromTrade" varStatus="index">
                                        <tr>
                                            <td class="serial">${index.index + 1}</td>
                                            <td>${fromTrade.fromStudentRealName}</td>
                                            <td>${fromTrade.fromStudentNum}</td>
                                            <td>${fromTrade.fromSkillTitle}</td>
                                            <td>${fromTrade.toStudentRealName}</td>
                                            <td>${fromTrade.toStudentNum}</td>
                                            <td>${fromTrade.toSkillTitle}</td>
                                            <td><f:formatDate value="${fromTrade.createTime}" pattern="yyyy-MM-dd HH:mm:ss"/></td>
                                            <td>${fromTrade.state == 0 ? "请求交换" : "已同意"}</td>
                                            <td>
                                                <a href="${ctx}/trade/delete?id=${fromTrade.id}" class="btn btn-danger">删除</a>
                                            </td>
                                        </tr>
                                    </c:forEach>
                                    <c:forEach items="${toTrades}" var="toTrade" varStatus="index">
                                        <tr>
                                            <td class="serial">${index.index + 1}</td>
                                            <td>${toTrade.fromStudentRealName}</td>
                                            <td>${toTrade.fromStudentNum}</td>
                                            <td>${toTrade.fromSkillTitle}</td>
                                            <td>${toTrade.toStudentRealName}</td>
                                            <td>${toTrade.toStudentNum}</td>
                                            <td>${toTrade.toSkillTitle}</td>
                                            <td><f:formatDate value="${toTrade.createTime}" pattern="yyyy-MM-dd HH:mm:ss"/></td>
                                            <td>${toTrade.state == 0 ? "请求交换" : "已同意"}</td>
                                            <td>
                                                <c:if test="${toTrade.state == 0}">
                                                    <a href="${ctx}/trade/update?id=${toTrade.id}"
                                                       class="btn btn-info">同意</a>
                                                </c:if>
                                                <a href="${ctx}/trade/delete?id=${toTrade.id}" class="btn btn-danger">删除</a>
                                            </td>
                                        </tr>
                                    </c:forEach>
                                    </tbody>
