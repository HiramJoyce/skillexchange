                    ...
                    <div class="col-xl-12">
                        <div class="card">
                            <div class="card-header">
                                ...
                                <a class="btn btn-success btn-sm" href="${ctx}/student/skill">添加</a>
                            </div>
                            <div class="table-stats order-table ov-h">
                                <table class="table" style="margin-bottom: 0;">
                                    ...
                                    <tbody>
                                    <c:forEach items="${skills}" var="skill" varStatus="index">
                                        <tr>
                                            <td class="serial">${index.index + 1}</td>
                                            <td>${skill.studentNum}</td>
                                            <td>${skill.studentRealName}</td>
                                            <td>${skill.title}</td>
                                            <td>${skill.content}</td>
                                            <td>${skill.askFor}</td>
                                            <td><f:formatDate value="${skill.createTime}" pattern="yyyy-MM-dd HH:mm:ss"/></td>
                                            <td>${skill.state}</td>
                                            <td>
                                                <a href="${ctx}/student/skill?id=${skill.id}" class="btn btn-info">修改</a>
                                                <a href="${ctx}/skill/delete?id=${skill.id}" class="btn btn-danger">删除</a>
                                            </td>
                                        </tr>
                                    </c:forEach>
                                    </tbody>
